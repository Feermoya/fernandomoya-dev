...
import { updateServiceTariff } from '@/lib/panel/repositories/servicesWrite';
import type { UpdateTariffInput } from '@/lib/panel/services/validate';
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  let payload: UpdateTariffInput;
  try {
    payload = (await request.json()) as UpdateTariffInput;
  } catch {
    return panelJson({ ok: false, error: 'JSON inválido.' }, 400);
  }
  const result = await updateServiceTariff(access.supabase, payload);
