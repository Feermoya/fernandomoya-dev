...
import { deactivateService } from '@/lib/panel/repositories/servicesWrite';
import type { DeactivateServiceInput } from '@/lib/panel/services/validate';
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  let payload: DeactivateServiceInput;
  try {
    payload = (await request.json()) as DeactivateServiceInput;
  } catch {
    return panelJson({ ok: false, error: 'JSON inválido.' }, 400);
  }
  const result = await deactivateService(access.supabase, payload);
